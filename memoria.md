...
- **Robustez / anti-crash**:
  ...
  - **Origen / postMessage (SOLUCIÓN FINAL: youtube-nocookie.com + host)**: el iframe del embed se comunica con la página padre vía `postMessage`. Para que los mensajes de estado (`onStateChange`/`onVideoEnded`) lleguen al puente: **Base URL de `loadDataWithBaseURL` y `playerVars.origin` apuntan al dominio oficial de incrustación `https://www.youtube-nocookie.com`**, con `enablejsapi: 1` y `widget_referrer` al mismo dominio, y además el **`host` de `YT.Player` apunta a `https://www.youtube-nocookie.com`** (sin él, `www-widgetapi.js` envía los eventos `postMessage` por defecto a `www.youtube.com`, que no coincide con el origen del WebView → `Failed to execute 'postMessage' ... target origin 'https://www.youtube.com' does not match recipient origin 'https://www.youtube-nocookie.com'`). **Historial del tanteo**: usar `https://www.youtube.com` como base dispara el **error 152-4 (suplantación)**, y un dominio propio (`com.cuetotech.vibetube`) hacía que el embed cayera al fallback de `youtube.com` → `postMessage` bloqueado y autoplay roto. La llamada de cambio de canción usa guard `window.player && typeof player.loadVideoById === 'function'` + `loadVideoById(...)` + `playVideo()`.
...
## Pendiente / ideas
- **Autoplay (retomar)**: el flujo completo `onVideoEnded` → `playNextTrack` → `loadVideoById` → reproducción de la siguiente canción **sigue sin reproducir** en el emulador/dispositivo, aunque la API Web carga vídeos sin errores de origen desde DevTools. Retomar la depuración con `chrome://inspect` + logcat (`VibeTubePlayer`).
- ~~Reproducción automática de la siguiente canción al terminar.~~ **Parcial**: la lógica de avance (bucle) y el puente Kotlin/JS están implementados, pero falta confirmar el flujo completo en dispositivo.
