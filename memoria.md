...
### Iteración 28 — Pantalla completa del reproductor + eliminación del botón "Añadir a lista" (05-08-2026)
- **Objetivo (especificación del usuario)**: (1) eliminar el botón superior derecho "Añadir a lista" del reproductor; (2) habilitar la pantalla completa del vídeo: al entrar se fuerza orientación horizontal (landscape) y al salir se vuelve a vertical (portrait).
- **Eliminación del botón "Añadir a lista"**: quitados el bloque `if (showAddSong) { IconButton(...) }` y los parámetros `onAddSong`/`showAddSong` de `YouTubePlayerView` (junto a los imports `Icons.Filled.Add`/`IconButton`/`CircleShape`); en `PlaylistsScreen` se retiran `onAddSong = viewModel::openAddSongDialog` (call site del reproductor), el parámetro `onAddSong` de `PlaylistDetail` y el paso `showAddSong = !isSaved`. El string `player_add_to_playlist` se conserva porque lo sigue usando `HomeScreen` (pestaña "Guardadas"). `openAddSongDialog` del ViewModel sigue activo para esa pantalla.
- **`player/YouTubePlayerView.kt` — pantalla completa**:
  - `class FullscreenHolder`: estado de la vista personalizada (`active`, `container`, `callback`, `previousRequestedOrientation`) que `onHideCustomView` y la limpieza reutilizan.
  - `WebChromeClient` con `onShowCustomView(view, callback)`: añade la vista del vídeo a un `FrameLayout` negro a pantalla completa sobre el `decorView`, oculta el reproductor pequeño de fondo (`View.GONE`), guarda la orientación previa y fuerza `ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE`; `onHideCustomView()` invoca `exitFullscreen(...)` y actualiza `isFullscreen`.
  - `exitFullscreen(...)` idempotente: restaura la orientación previa, retira el contenedor del `decorView`, llama `onCustomViewHidden()`, vuelve a mostrar el reproductor pequeño y limpia el estado. También se invoca desde `onDispose` para no dejar la app atascada en landscape si el composable se destruye en pleno fullscreen.
  - `BackHandler(enabled = isFullscreen)`: el botón Atrás del sistema (o gesto) sale del fullscreen ejecutando `player.exitFullscreen()` (que dispara `onHideCustomView`) en vez de cerrar la pantalla.
  - `Context.findActivity()`: desenrolla `ContextWrapper` para localizar la `Activity` contenedora (orientación + decorView).
  - **Decisión de diseño**: se mantienen visibles las barras de sistema durante el fullscreen. Ocultarlas (flags `SYSTEM_UI_FLAG_*` primero, luego `WindowInsetsControllerCompat`) disparaba en el emulador el overlay de confirmación del modo inmersivo del sistema, que se quedaba con el foco y se **tragaba el botón Atrás**, impidiendo que el `BackHandler` de la app saliera del fullscreen. El vídeo sigue llenando la pantalla y girando; las barras quedan encima (compromiso aceptable frente a la fiabilidad del Atrás).
- **Verificación en emulador (API 37)**: sin botón "Añadir a lista" en el reproductor (dump de UI limpio). Pantalla completa: tap en el botón del player → `onShowCustomView` + `mRotation=ROTATION_90` (landscape) ✅; salida con el botón del player ("Exit full screen") → `onHideCustomView` + `ROTATION_0` (portrait) ✅; salida con el **botón Atrás del sistema** → `BackHandler` → `player.exitFullscreen()` → `onHideCustomView` + `ROTATION_0` ✅ (sin overlay inmersivo, foco siempre en la app).
- Verificación: `./gradlew :app:assembleDebug :app:testDebugUnitTest` en verde (20 tests, 0 fallos).

### Iteración 29 — PlaybackService adaptado a Android Auto con las listas reales (Firestore) (11-08-2026)
- **Objetivo (especificación del usuario)**: sustituir la lista estática de prueba de `PlaybackService` por las listas reales del usuario logueado, para que Android Auto muestre el contenido real de la app.
- **AndroidManifest.xml**: `PlaybackService` ahora declara los intent-filters `android.media.browse.MediaBrowserService` **y** `androidx.media3.session.MediaLibraryService` (Android Auto busca el primero para la navegación por librería), y se añade el `<meta-data>` de `com.google.android.gms.car.application` apuntando a `res/xml/automotive_app_desc.xml`.
- **`data/PlaylistRepository.kt`**: nueva `suspend fun getUserPlaylists(ownerId)`: consulta única (una sola lectura, sin listener) a la colección `playlists` filtrando por `ownerId`; pensada para usarse fuera de la UI (servicios).
- **`player/PlaybackService.kt`**:
  - Constantes nuevas: `TAG_MEDIA = "VibeTubeMedia"`, `AUTH_TIMEOUT_MS = 2_000` (tiempo máximo de espera de la sesión de auth) y `AUTH_RETRY_DELAY_MS = 100` (reintento de comprobación).
  - `ROOT_ID = "root_vibetube"`. El `mediaId` de cada carpeta es el `id` real de la playlist; el de cada canción es su `youtubeId` (lo usa `onAddMediaItems`).
  - `onGetLibraryRoot` intacto (raíz `root_vibetube`).
  - `onGetChildren` reescrito sobre `SettableFuture<LibraryResult<ImmutableList<MediaItem>>>` que **solo se resuelve al terminar la carga real** (se acuerda no resolverlo antes de que Firestore devuelva datos). Todo el trabajo va en el scope del servicio con try/catch: en éxito → `LibraryResult.ofItemList(...)`; en error → lista vacía + `Log.e(TAG_MEDIA, ...)`. Se eliminó la antigua rama que devolvía la lista estática de prueba.
  - `ROOT_ID` → `loadPlaylistFolders()`: `awaitCurrentUserId()` (espera hasta 2 s reintentando cada 100 ms a que `auth.currentUser?.uid` no sea null) + `getUserPlaylists(uid)` → carpetas con `isBrowsable=true`, `isPlayable=false` y `setFolderType(FOLDER_TYPE_PLAYLISTS)`.
  - Cualquier otro parentId → `loadPlaylistSongs(playlistId)`: `getPlaylist(id)?.tracks` → MediaItems con `setMediaId(youtubeId)`, `setDurationMs` (⚠️ en Media3 1.10.1 es `setDurationMs(Long)`, no `setDuration`), `isBrowsable=false`, `isPlayable=true`.
  - `notifyRootIfPreviouslyEmpty(children)`: si la raíz anterior estaba vacía y ahora tiene hijos → `mediaLibrarySession?.notifyChildrenChanged(ROOT_ID, 0, null)` para que Android Auto refresque su árbol sin reiniciar; mantiene `lastRootChildrenCount`.
  - `onAddMediaItems` (extracción de audio con NewPipe) intacto; `onDestroy` cancela `serviceJob`.
  - Logs `Log.d(TAG_MEDIA, ...)`: `onGetChildren(root): userId=...`, `N listas del usuario ...`, `sin sesión tras 2000 ms → lista vacía`, `la lista anterior estaba vacía → notifyChildrenChanged(...)`, y `onGetChildren(<id>): N canciones`.
- **Validación en emulador (API 37)** con test instrumentado temporal (`PlaybackServiceBrowseTest`, borrado tras validar): se logueaba una cuenta de prueba (o se creaba si no existía), se sembraba una playlist con 1 canción si el usuario no tenía, y vía `MediaBrowser.Builder(...).buildAsync()` + `getLibraryRoot` + `getChildren` se recorría la raíz y la primera lista. Resultado en `logcat`:
  - `userId=tk4XxjKrYsTSY8wJFnZ2oDyerNJ3` → `1 listas del usuario` → `la lista anterior estaba vacía → notifyChildrenChanged(root_vibetube)` → `1 canciones`. La raíz y la primera playlist devuelven contenido real de Firestore ✅.
  - Detalle del test: `MediaBrowser` (de Media3 1.10.1) exige que sus métodos se llamen desde el hilo de la aplicación (looper con el que se construyó); el hilo del test debe esperar con `.get()` mientras las llamadas se publican en un `HandlerThread` propio.
- **Causa probable de la pantalla vacía reportada en Android Auto**: el `SettableFuture` se resolvía antes de que Firestore devolviera las listas (o antes de que la sesión de auth estuviera lista). Con la espera a `currentUser` (timeout 2 s) + resolución del futuro solo al terminar la carga + `notifyChildrenChanged` si la raíz pasó de vacía a no vacía, el árbol de Android Auto se rellena solo.
- Verificación: `./gradlew :app:assembleDebug :app:testDebugUnitTest` en verde (20 tests, 0 fallos).

### Iteración 30 — Android Auto: carátulas, cola completa en ExoPlayer y control del volante (11-08-2026)
- **Objetivo (especificación del usuario)**: corregir en `PlaybackService.kt` que la reproducción no pasaba de canción, que no se veían las carátulas y que los botones del volante no funcionaban.
- **Metadatos visuales (carátulas)**: al construir cada `MediaItem` de canción se asigna siempre la carátula con `.setArtworkUri(Uri.parse(urlCaratula))` en el `MediaMetadata.Builder()`:
  - `Song.kt`: nueva propiedad calculada `imageUrl` → `https://i.ytimg.com/vi/{youtubeId}/hqdefault.jpg` (miniatura hqdefault de YouTube).
  - `Song.asMediaItem(playlistId)` (usado por `onGetChildren`): `setArtworkUri(Uri.parse(imageUrl))` junto a título/artista/duración.
  - `MediaItem.ensureArtwork()` (usado por `onAddMediaItems`): si el ítem recibido no trae carátula, se la añade a partir del `youtubeId`; se aplica siempre, incluso cuando la extracción de audio falla.
- **Cola completa en ExoPlayer (`onSetMediaItems` sobrescrito)**: cuando el usuario selecciona una canción dentro de una lista en Android Auto, en lugar de enviar solo ese ítem aislado a la cola (que impedía saltar de pista y el autoplay) se carga la lista completa y se posiciona el reproductor en el ítem seleccionado:
  - El `mediaId` de cada canción pasa a ser `"playlistId:youtubeId"` (`MEDIA_ID_SEPARATOR = ":"`), de modo que `splitMediaId()` permite saber a qué lista pertenece la canción pulsada y volver a cargarla completa desde Firestore.
  - `onSetMediaItems` devuelve `MediaItemsWithStartPosition(listaCompleta, índiceSeleccionado, 0L)` (la sesión aplica `Player.setMediaItems(items, index, pos)`, equivalente a `player.seekTo(index, 0L)`) y, además, aplica la cola directamente con `mediaLibrarySession.player.setMediaItems(...)` para garantizar el montaje con independencia del flujo interno de la sesión.
  - `onAddMediaItems` (comando de añadir a la cola) resuelve igualmente todos los ítems antes de devolverlos.
- **Bugs reales encontrados y corregidos durante la validación**:
  - **ExoPlayer exige URI en TODOS los ítems para montar una cola**: `Player.setMediaItems` lanza `IllegalArgumentException` en `DefaultMediaSourceFactory.createMediaSource` si algún ítem no tiene URI → la cola nunca se montaba y no se pasaba de canción. Por eso `resolveQueue()` resuelve las URIs de audio de **todas** las canciones (secuencial, activo primero, luego el resto en orden) ANTES de devolver/montar la cola; el enfoque "lazy con replaceMediaItem en segundo plano" se descartó porque choca con este requisito.
  - **La extracción del servicio era ingenua**: usaba NewPipe directamente sin `NewPipe.init` (fallaba al instante con "not initialized"), sin preseleccionar la versión del cliente WEB (falla en redes bloqueadas) y sin preferir audio puro. Se sustituye por `YouTubeStreamResolver.resolveSingleAudioUrl()` (el extractor validado de la app: init + client version preseleccionada + formato M4A/WEBMA/OPUS preferido + fallback a vídeo progresivo). En emulador: `fetchPage OK`, 5 streams, WEBMA_OPUS elegido.
  - **Timeout que no interrumpía `fetchPage()`**: `withTimeoutOrNull` alrededor de `withContext(Dispatchers.IO)` no puede abortar una llamada de red bloqueante (la cancelación espera a que termine). Se crea `extractorScope = CoroutineScope(Dispatchers.IO + SupervisorJob())` desacoplado y la extracción corre como `extractorScope.async { ... }` con `withTimeoutOrNull(AUDIO_EXTRACTION_TIMEOUT_MS = 10_000)` alrededor del `await()`: si se agota el tiempo se devuelve null y la cola se monta igual; el scope se cancela en `onDestroy`.
- **`playWhenReady`**: en `onCreate()` se fija `player.playWhenReady = true` para reproducción automática en cuanto la sesión reciba la cola.
- **Validación en emulador (API 37)** con test instrumentado temporal (`PlaybackServiceQueueTest`, borrado tras pasar): cuenta de prueba + lista con 3 canciones → navegación raíz → carpeta → 3 canciones con `artworkUri` no nulo ✅; `MediaBrowser.setMediaItem(songs[1])` → `onSetMediaItems` monta la cola completa → `getMediaItemCount()==3` y `getCurrentMediaItemIndex()==1` ✅. Los `MediaBrowser`/`MediaController` de Media3 1.10.1 exigen que sus métodos se llamen desde el looper con el que se construyeron (test con `HandlerThread` propio; el hilo del test espera con `.get()`).
- Commit: `58f7b75` `fix(auto): carátulas, cola completa en ExoPlayer y control del volante en Android Auto` (pusheado a `origin main`).
- Verificación: `./gradlew :app:assembleDebug :app:testDebugUnitTest` en verde (20 tests, 0 fallos).
...
### Iteración 32 — Fix Android Auto "no disponible", latencia handoff y reinicio de canción (24-08-2026)
- **Síntomas reportados**: (1) Android Auto muestra "no disponible ahora mismo"; (2) al bloquear el móvil, tarda 10-20s en abrir el reproductor en segundo plano; (3) la canción se reinicia desde el principio a los pocos segundos de empezar.
- **Causa raíz del Android Auto**: `onSetMediaItems` devolvía items **sin URIs de audio** → el framework los pasaba a `player.setMediaItems()` → `DefaultMediaSourceFactory.createMediaSource()` lanzaba `IllegalStateException` (no puede crear `MediaSource` sin URI) → el servicio moría y Android Auto mostraba "no disponible". `onAddMediaItems` no se llamaba después de `setMediaItems` (solo se invoca con `addMediaItems`), por lo que la resolución en background nunca llegaba a ExoPlayer.
- **Causa raíz del reinicio**: el servicio arrancaba en posición 0 (por `setMediaItems(..., 0L)`) y el handoff no leía la posición del WebView, así que al delegar el audio al servicio la canción volvía al principio.
- **Fix `onSetMediaItems`**: ahora resuelve URLs en paralelo vía `YouTubeStreamResolver.resolveAudioUrls()` ANTES de devolver los ítems. Cuando items ya tienen URI (llamada del teléfono via `syncPlaylist`), pasa directamente. Cuando no tienen URI (Android Auto), resuelve y devuelve items **con URI válida**.
- **Fix `handOffToService`**: ahora espera a que `isActive` sea true (máx 30s) antes de pausar el WebView, para no perder la transición si `syncPlaylist` aún resolvía URLs. Además, lee la posición del WebView (con timeout 1.5s) y hace `playFromPosition(position)` para que el servicio continúe desde donde estaba el WebView, no desde el principio.
- **Fix `readWebPositionWithTimeout`**: wrapper `suspendCancellableCoroutine` con `withTimeoutOrNull(1.5s)` sobre `webPlayer.currentPosition()` para que la evaluación JS no bloquee el handoff si la pantalla se está apagando.
...
### Iteración 34 — Fixes defensivos: crash del servicio y orden de inicialización (25-08-2026)
- **Síntomas reportados**: (1) Android Auto sigue mostrando "VibeTube no está funcionando en ese momento"; (2) en el teléfono no aparece el icono de shuffle en la notificación.
- **Hipótesis**: el `PlaybackService` crashea silenciosamente al arrancar, provocando que tanto Android Auto como la notificación no funcionen. Sin logcat no se puede confirmar el crash exacto, pero se aplican fixes defensivos que cubren los puntos de fallo más probables.
- **Fix 1 — Orden de inicialización**: `setMediaNotificationProvider(CustomNotificationProvider())` se movió ANTES de `MediaLibrarySession.Builder.build()`. Media3 espera que el provider esté registrado antes de que la sesión se construya; si la sesión intenta crear la notificación interna durante `build()` y el provider no existe, puede lanzar una excepción no controlada.
- **Fix 2 — Try-catch en `onCreate()`**: todo el cuerpo de `onCreate()` del servicio se envuelve en try-catch con `Log.e(TAG_MEDIA, ...)`. Si algo falla (ExoPlayer, notification provider, session builder), el servicio no muere silenciosamente y el error queda registrado en logcat.
...
### Iteración 42 — Fix "Session operation failed" (NPE por ítems sin URI + cancelación cruzada de corrutinas) verificado con test de instrumentación en emulador (14-09-2026)
- **Síntoma**: tras la Iteración 41, el error "No se ha podido cargar tu selección" persistía en Android Auto; además, al probar el flujo real con un test de instrumentación en el emulador (MediaController → `setMediaItems` con un solo item `playlistId:youtubeId`) el `onSetMediaItems` resolvía rápido pero **nunca llegaba a reproducir** (timeline caído a 0 items, estado ENDED).
- **Diagnóstico con logs reales del emulador** (logcat, tags `VibeTubeMedia` / `VibeTubeStream` / `MediaSessionStub`):
  1. `onSetMediaItems` respondía en ~12 ms (bien, sin timeout de Android Auto), pero el item se entregaba **sin URI** (`uri=-1`).
  2. `playlistDef` (Firestore) lanzaba `FirebaseFirestoreException: PERMISSION_DENIED` (no hay sesión de usuario en el test) → como `urlDef` y `playlistDef` eran hijos `async` del mismo `launch`, el fallo cancelaba el **padre y a la hermana** (`JobCancellationException: Parent job is Cancelling`): la resolución NewPipe del target quedaba descartada.
  3. El catch entregaba el `MediaItem` original sin `LocalConfiguration` → `MediaSessionStub` → `ExoPlayerImpl.setMediaItems` → `DefaultMediaSourceFactory.createMediaSource` → **`NullPointerException`** → `Session operation failed` → limpieza del timeline (items=0, ENDED). Es el "No se ha podido cargar tu selección" de Android Auto.
  4. La extracción NewPipe sí terminaba bien (~4,8 s) pero su resultado se tiraba por la cancelación.
- **Fix 1 — Aislamiento de corrutinas en FASE 1 de `onSetMediaItems`**: cada `async(Dispatchers.IO)` (`urlDef` y `playlistDef`) rodea su cuerpo con `try/catch` que devuelve `null` en vez de propagar la excepción. Un fallo de Firestore ya no cancela la URL hermana ni lanza `JobCancellationException`.
- **Fix 2 — Nunca entregar ítems sin URI al reproductor**: las respuestas de `onSetMediaItems` se filtran con `.filter { it.localConfiguration?.uri != null }` (ExoPlayer + `DefaultMediaSourceFactory` lanza NPE con ítems sin URI). Si la playlist está lista se entregan solo los ítems con URI; si no hay URI lista se entrega igualmente el item único recién resuelto (con `setUri`); el resto se resuelve en background.
- **Fix 3 — Crecimiento progresivo de la cola solo con URIs**: `resolveRemainingInBackground` ya no usa `backfillTimelineUris` (eliminado); resuelve URLs pendientes y amplía el timeline únicamente con ítems que tengan URI (`replaceMediaItems` solo si `playable.size > count` actual).
- **Fix 4 — `onAddMediaItems`**: el catch ahora entrega `prepareItemsFromCache(mediaItems)` en vez de la lista cruda (mismo riesgo de NPE).
- **Nuevo test de reproducción**: `app/src/androidTest/java/com/cuetotech/vibetube/AndroidAutoSelectionTimeoutTest.kt` conecta un `MediaController` al `PlaybackService` (igual que Android Auto), envía un item único y mide: tiempo hasta timeline, hasta URI resuelta y hasta READY/BUFFERING; vuelca logs a logcat con tag `VibeTubeAATest`. Ejecución: `./gradlew connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.cuetotech.vibetube.AndroidAutoSelectionTimeoutTest`.
- **Verificación en emulador (Medium_Phone_API_36.1, API 36)**: ANTES del fix `uri=-1` y timeline colapsado; DESPUÉS del fix `URI resuelta a t=5708ms` con URL real de googlevideo, `onMediaItemTransition` + `BUFFERING`, timeline estable (items=1), sin NPE ni "Session operation failed". NewPipe resolvió el target (~4,8 s) y ahora el resultado se entrega al reproductor.
- **Verificación de reproducción REAL (test ampliado, 2ª ejecución)**: el test ahora espera `STATE_READY` (no BUFFERING optimista) y avance de posición como condición de éxito: `RESUMEN: total=8364ms | timeline=13ms | uri=5521ms | ready=7346ms | avanza=true | pos=1078ms | items=1`. La posición avanza 261→363→464→…→1077ms (muestras de 100ms), y el system `MediaSessionService` confirma `PLAYING(3)` con `position=0→3000→6002→9013→12014→15020` y buffered position creciente → el audio se decodifica y reproduce de verdad. El "No se ha podido cargar tu selección" queda resuelto (el flujo completo selección→finalización del futuro con URI→READY→posición avanzando es de ~8,4 s, dominado por la extracción NewPipe en frío; con URL en `urlCache` es casi instantáneo).
- Build: `./gradlew assembleDebug` → BUILD SUCCESSFUL.

### Iteración 43 — Notificación foreground INMEDIATA en el teléfono (placeholder silencioso + resolución asíncrona NewPipe + backfill de cola) verificado con test de instrumentación (14-09-2026)
- **Síntoma**: en el teléfono, al pulsar una canción y minimizar la app (o apagar la pantalla), la notificación del reproductor tardaba **~10 s** en aparecer porque `PlaybackController.syncPlaylist` resolvía con NewPipe TODAS las URLs de la cola (con `YouTubeStreamResolver.resolveAudioUrls`, que no tiene caché) ANTES de enviar `setMediaItems` al servicio; sin timeline el `MediaNotificationManager` no mostraba nada.
- **Diseño (mismo patrón aplicado a Android Auto en Iteración 42, ahora para el teléfono)**:
  1. **FASE 1a — Notificación inmediata**: al pulsar, se envía al instante un item provisional con **URI válida** (`rawresource:///silent_track`, WAV silencioso de 0,25 s empaquetado en `res/raw/silent_track.wav`) y la **metadata real en caché** (título, artista, artwork). `prepare()`/`play()` ya muestran la notificación foreground (media3: `shouldShowNotification` solo exige timeline no vacía y estado != IDLE). Evita además el NPE de ExoPlayer con ítems sin URI.
  2. **FASE 1b — Hot replace**: la URL real del target se resuelve en `Dispatchers.IO` en background y `replaceMediaItem(0, realTarget)` sustituye el placeholder en caliente.
  3. **FASE 2 — Backfill**: el resto de la cola se resuelve en background y se va añadiendo en orden con `addMediaItem`, actualizando el mapeo `vmToServiceIndex` (@Volatile).
- **Cambios en `PlaybackService.kt`**: añadido `Player.COMMAND_CHANGE_MEDIA_ITEMS` a la sesión (cubre `addMediaItem`/`replaceMediaItem`; en media3 1.10.1 NO existen `COMMAND_ADD_MEDIA_ITEM`/`COMMAND_REPLACE_MEDIA_ITEM`); `resolveQueueFast`/`onSetMediaItems` (single-item)/`onAddMediaItems`: se **salta NewPipe** cuando el ítem ya trae URI en `localConfiguration` (evita re-extracción y timeouts).
- **Cambios en `PlaybackController.kt`**: `syncPlaylist` reescrito (placeholder inmediato → `inflateQueue` en `backgroundScope`); `buildPlaceholderItem`, `resolveFirstPlayable` (target o siguiente válida, usando `urlCache`), `playlistOrderAfter`, `urlCache` (ConcurrentHashMap), `playbackGeneration` (invalida backfills obsoletos), `realTrackDef` (CompletableDeferred completado al reemplazar), `awaitFirstTrackReady()` (máx 15 s para que `play()`/`playFromPosition()` no reproduzcan silencio), `stop()`/`release()` con bump de generación + cancelación.
- **Fix intermedio (indispensable)**: el placeholder usaba el datasource `DefaultHttpDataSource` (solo HTTP), que no entiende `rawresource://` → `MalformedURLException: unknown protocol: rawresource` y el player quedaba en BUFFERING sin reproducir. Cambiado a `DefaultDataSource.Factory(context, httpFactory)` (media3 gestiona `rawresource://` con `RawResourceDataSource` y delega http/https al factory HTTP con user agent y cross-redirects).
- **Nuevo test**: `PhoneForegroundNotificationDelayTest.kt` (tag `VibeTubePhoneTest`): conecta `PlaybackController` con 2 pistas reales (`startPlaying=true`) y un `MediaController` observador; miden: placeholder en timeline (sin condición de URI para observadores externos), notificación con el título en `NotificationManager.activeNotifications`, backfill a 2 items, y **reproducción real** (posición > 1000 ms; el placeholder dura solo 250 ms).
- **Verificación en emulador (Medium_Phone_API_36.1, API 36)**: `RESUMEN: placeholder=4518ms | notificacion=4518ms | playingReal=8719ms | backfill=6288ms`. La notificación aparece **al instante con la pista seleccionada** (antes ~10 s), el placeholder se reemplaza (posición avanza 481→582→…→1089 ms de la canción real de YouTube), la cola se infla a 2 ítems, y NO hay errores de reproducción (`ExoPlaybackException` en logcat). Sin NPE, sin cancelaciones, `syncPlaylist` devuelve true sin bloquear en NewPipe.
- Build: `./gradlew assembleDebug` → BUILD SUCCESSFUL.

### Iteración 44 — Fix "la primera canción no suena al bloquear el teléfono": reanudación explícita tras el hot replace (14-09-2026)
- **Síntoma**: la notificación aparecía al instante, pero al bloquear el teléfono (o apagar la pantalla) la primera canción se quedaba en silencio hasta pasar a la siguiente manualmente.
- **Causa raíz**: el hot replace del placeholder (WAV silencioso de 250 ms) usaba `controller.replaceMediaItem(0, realTarget)` SIN preparar ni reanudar explícitamente. Si el `silent_track` ya había terminado (STATE_ENDED por su duración de 250 ms y/o `playWhenReady` levantado durante el arranque), ExoPlayer quedaba anclado en ENDED: `play()`/`playFromPosition()` del handoff no reiniciaba el ítem y solo un avance manual (next) reactivaba la reproducción.
- **Fix 1 — Hot replace con transición COMPLETA (`PlaybackController.inflateQueue`)**: en sustitución de `replaceMediaItem`, se ejecuta la secuencia `controller.setMediaItem(realTarget)` → `controller.prepare()` → `controller.playWhenReady = startPlaying || controller.playWhenReady`. Con `setMediaItem` el player resetea posición y timeline y con `prepare()` sale SIEMPRE de STATE_ENDED; `playWhenReady` se preserva según la intención: si arrancó con audio o el usuario ya había reanudado (handoff/bloqueo durante la resolución), reanuda al instante; si la app sigue en primer plano con el WebView sonando, queda pausado hasta el handoff.
- **Fix 2 — Defensa contra ENDED residual (`play()`, `playFromPosition()`)**: tras `awaitFirstTrackReady()`, si `playbackState == STATE_ENDED` se hace `seekTo(currentMediaItemIndex, 0L)` antes de `play()`, de modo que un posible anclaje residual del silent_track nunca deje la pista 1 muda. Esto previene el "fin de pista prematuro": el ENDED del placeholder no dispara salto automático ni deja el servicio detenido (el `PlaybackService` no tiene lógica de parada en ENDED; solo registra `onPlaybackStateChanged`).
- **Tests (PhoneForegroundNotificationDelayTest, ambos PASS)**:
  - `immediateNotification_thenRealPlaybackAndQueueBackfill` (`startPlaying=true`): log de hot-replace `pista real reemplazada y reanudada (playWhenReady=true)`; `RESUMEN: placeholder=7304ms | notificacion=7304ms | playingReal=9761ms | backfill=8543ms`.
  - NUEVO `resumeAfterLock_playsRealTrackWithoutManualSkip` (reproduce el escenario exacto del teléfono): `syncPlaylist(startPlaying=false)` + `play()` emulando el bloqueo/handoff; `RESUMEN lock: playAt=1970ms | playingReal=4237ms` → pos avanza a 1035 ms de la PRIMERA pista sin ningún salto manual.
- **Sin regresión en Android Auto**: `AndroidAutoSelectionTimeoutTest` → `RESUMEN: total=7113ms | timeline=12ms | uri=4981ms | ready=6099ms | avanza=true | pos=1059ms | items=1`.
- Build: `./gradlew assembleDebug` → BUILD SUCCESSFUL.
